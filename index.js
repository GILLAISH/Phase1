 ...
 */
let Users = [
    { // Sample Owner User -> email: owner@localhost, password: password
        name: 'Owner',
    ...
    { // Sample Coworker User -> coworker@localhost, password: password
        name: 'Coworker',
        email: 'coworker@localhost',
        password: 'password',
        phone: '1234567890',
        role: 'coworker'
    }];
...
app.post('/register', (req, res) => { // Create User
...
app.post('/owner/property/create', checkAuth, (req, res) => {
    if (req.session.user.role === 'owner') {
        propertyID++;
        const property = {
            id: propertyID,
...
app.post('/owner/property/:id/edit', checkAuth, (req, res) => {
    if (req.session.user.role === 'owner') {
...
app.get('/owner/property/:id/delete', checkAuth, (req, res) => {
    if (req.session.user.role === 'owner') {
        if(Properties !== undefined && Properties.hasOwnProperty(req.session.user.email)) {
            let removeIndex;
...
app.post('/owner/workspace/create', checkAuth, (req, res) => {
    if (req.session.user.role === 'owner') {
        workspaceID++;
        const workspace = {
            id: workspaceID,
...
app.post('/owner/workspace/:id/edit', checkAuth, (req, res) => {
    if(req.session.user.role === 'owner') {
        Properties[req.session.user.email].forEach(property => {
            if(property.hasOwnProperty('workspaces')){
                property.workspaces.forEach(workspace => {
...
app.get('/owner/workspace/:id/delete', checkAuth, (req, res) => {
    if (req.session.user.role === 'owner') {
        if(Properties !== undefined && Properties.hasOwnProperty(req.session.user.email)) {
            Properties[req.session.user.email].forEach(property => {
                if (property.hasOwnProperty('workspaces')){
...
app.get('/search', checkAuth, (req, res) => {
    if (Object.keys(req.query).length === 0){
        getSpaces();
    } else if (Object.keys(req.query).length === 1 && (propertyQueries.includes(Object.keys(req.query)[0]) || workspaceQueries.includes(Object.keys(req.query)[0]))) {
        getSpaces();
